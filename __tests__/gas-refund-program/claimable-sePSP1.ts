...
describe('claimable sePSP1', () => {
  ...
  describe('assert correct sePSP1 balance', () => {
    ...
    test(`check balance before and after claiming`, async () => {
      ...
      const account = '0x81ea04c179b82d31662994a68b2582ad9d860f88'.toLowerCase();
      ...
      const timestampByBlock = await fetchBlocksTimestamps({ chainId, blockNumbers: [startBlock, endBlock] });
      ...
      const balanceBeforeClaim = claimableSePSP1Tracker.getBalance(startTimestamp, account);
      const balanceAfterClaim = claimableSePSP1Tracker.getBalance(endTimestamp, account);
