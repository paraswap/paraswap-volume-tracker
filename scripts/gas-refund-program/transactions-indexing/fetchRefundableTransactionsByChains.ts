...
export async function fetchRefundableTransactionsByChain(chainId: number) {
  console.assert(GRP_SUPPORTED_CHAINS.includes(chainId));

  const _lastEpochRefunded = await getLatestEpochRefunded(chainId);
