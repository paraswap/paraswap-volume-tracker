...
export async function fetchRefundableTransactions({
  ...
  async function filterFormatAndStoreRefundableTransactions(transactions: CovalentGasRefundTransaction[], computeRefundPercent: (epoch: number, totalPSPorTotalParaboostScore: string) => number | undefined) {
    const refundableTransactions: GasRefundTransactionDataWithStakeScore[] =
              transactions
                .map(transaction => {
                  ...
                  const refundPercent = computeRefundPercent(epoch, totalStakeAmountPSP);
  ...
  // fetchParaswapV6StakersTransactions
  return (
    await Promise.all([
      ...
      //     const refundableTransactions = await filterFormatAndStoreRefundableTransactions(transactions, (epoch, totalScore) => {
      //       const result = contractAddress === MIGRATION_SEPSP2_100_PERCENT_KEY
      //                 ? 1 // 100%
      ...
      ...Array.from(AUGUSTUS_SWAPPERS_V6_OMNICHAIN).map(
        async contractAddress => {
          ...
          const allStakersTransactionsDuringEpoch = await fetchParaswapV6StakersTransactions({
            epoch:epochNewStyle,
            chainId,            
