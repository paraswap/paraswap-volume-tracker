...
const TokenWhitelist: TokenWhitelist = {
  1: {
    ...
    '0x0391d2021f89dc339f60fff84546ea23e337750f': [ // BarnBridge Governance Token (BOND)
      {
        from: 0
      }
    ],
    ...
    '0x2ba592f78db6436527729929aaf6c908497cb200': [ // Cream (CREAM)
      {
        from: 0
      }
    ],
    ...
    '0x4e15361fd6b4bb609fa63c81a2be19d873717870': [ // Fantom Token (FTM)
      {
        from: 0
      }
    ],
    ...
    '0x4fe83213d56308330ec302a8bd641f1d0113a4cc': [ // NuCypher (NU)
      {
        from: 0
      }
    ],
    ...
    '0x6f259637dcd74c767781e37bc6133cd6a68aa161': [ // HuobiToken (HT)
      {
        from: 0
      }
    ],
    '0x7d1afa7b718fb893db30a3abc0cfc608aacfebb0': [ // Matic Token (MATIC)
      {
        from: 0
      }
    ],
    ...
    '0x86772b1409b61c639eaac9ba0acfbb6e238e5f83': [ // Indexed (NDX)
      {
        from: 0
      }
    ],
    '0x8e870d67f660d95d5be530380d0ec0bd388289e1': [ // Pax Dollar (USDP) "Announcement: Paxos Standard (PAX) Token is now rebranded as Pax Dollar (USDP)."
      {
        from: 0
      }
    ],
    ...
    '0xa3bed4e1c75d00fa6f4e5e6922db7261b5e9acd2': [ // Meta (MTA)
      {
        from: 0
      }
    ],
    '0xa47c8bf37f92abed4a126bda807a7b7498661acd': [ // Wrapped UST Token (UST)
      {
        from: 0
      }
    ],
    '0xad32a8e6220741182940c5abf610bde99e737b2d': [ // PieDAO DOUGH v2 (DOUGH)
      {
        from: 0
      }
    ],
    '0xada0a1202462085999652dc5310a7a9e2bf3ed42': [ // CoinShares Gold and Cryptoassets Index (CGI)
      {
        from: 0
      }
    ],
    ...
    '0xbc396689893d065f41bc2c6ecbee5e0085233447': [ // Perpetual (PERP)
      {
        from: 0
      }
    ],
    ...
    '0xc834fa996fa3bec7aad3693af486ae53d8aa8b50': [ // Convergence (CONV)
      {
        from: 0
      }
    ],
    ...
    '0xd2877702675e6ceb975b4a1dff9fb7baf4c91ea9': [ // Wrapped LUNA Token (LUNA)
      {
        from: 0
      }
    ],
    '0xd417144312dbf50465b1c641d016962017ef6240': [ // Covalent Query Token (CQT)
      {
        from: 0
      }
    ],
    ...
    '0xf1f955016ecbcd7321c7266bccfb96c68ea5e49b': [ // Rally (RLY)
      {
        from: 0
      }
    ],
    ...
    '0x123151402076fc819b7564510989e475c9cd93ca': [ // wrapped-DGLD (wDGLD)
      {
        from: 0
      }
    ],
    ...
    '0xcafe001067cdef266afb7eb5a286dcfd277f3de5': [ //  Paraswap (PSP)
      {
        from: 1637605614
      }
    ]
...
const isWhitelistedToken = (network: number, token: string, timestamp: number) => {
  if (!(network in TokenWhitelist)) throw new Error(`TokenWhitelist is not exist for network ${network}`);
  if (!(token.toLowerCase() in TokenWhitelist[network])) return false;
  if (!timestamp) return false;

  const ranges = TokenWhitelist[network][token.toLowerCase()];

  return ranges.some(({from, to}) => timestamp >= from && (to ? timestamp <= to : true));
}
...
export class VolumeTracker {
  ...
  constructor(
    ...
    this.takerAddresses = [AugustusV5Address[this.network], AugustusV4Address[this.network]].filter(a => !!a);
  ...
  async indexZeroXV4(
    ...
    const filteredLogs = rawLogs.filter(
      log =>
        log.args &&
        this.takerAddresses.some(a => a.toLowerCase() === log.args!.taker.toLowerCase())
    );
    logger.info(`Found ${filteredLogs.length} from ${rawLogs.length} logs for ZeroXV4 ${fromBlock}: ${toBlock}`);

    return await Promise.all(
      filteredLogs.map(async log => {
        const args = log.args!;
        const timestamp = await this.blockInfo.getBlockTimeStamp(log.blockNumber);
        if (!timestamp) throw new Error('Failed to get block timestamp for record');
        return {
          ...
          fromVolumeUSD: (await this._getTokenPrice(
            log.blockNumber,
            args.takerToken,
          ...
          toVolumeUSD: (await this._getTokenPrice(
            log.blockNumber,
            args.makerToken,
            new BigNumber(args.makerTokenFilledAmount.toString()),
          )).toFixed(DataType_USD_VALUE_DECIMALS),
          isWhitelisted: isWhitelistedToken(this.network, args.takerToken.toLowerCase(), timestamp)
            && isWhitelistedToken(this.network, args.makerToken.toLowerCase(), timestamp)
        };
      })
    );
  }
  ...
  async indexZeroXV2(
    ...
    const filteredLogs = rawLogs.filter(
      log =>
        log.args &&
        this.takerAddresses.some(a => a.toLowerCase() === log.args!.takerAddress.toLowerCase())
    );
    logger.info(`Found ${filteredLogs.length} from ${rawLogs.length} logs for ZeroXV2 ${fromBlock}: ${toBlock}`);
    ...
    return await Promise.all(
      filteredLogs.map(async log => {
        ...
        return {
          ...
          isWhitelisted: isWhitelistedToken(this.network, takerToken.toLowerCase(), timestamp)
            && isWhitelistedToken(this.network, makerToken.toLowerCase(), timestamp)
        };
      })
    );
  }
  ...
  async indexLatest() {
    ...
    try {
      if (!this.initBlock) {
        ...
        if (!this.initBlock) {
          logger.warn(`_indexLatest: unable to fetch block number for ${this.initTime}`);
          return;
        }
      }
      ...
      do {
        [fromBlock, toBlock] = await Database.sequelize.transaction(async (t) => {
          ...
          try {
            ...
            reportFailure = false;
            logger.info(`Unable to lock VolumeSyncStatus for network ${this.network}`);
            throw e;
          }
          if (!syncStatuses.length)
            throw new Error(`Didn't find sync status for network ${this.network}`);
          const syncStatus: VolumeSyncStatus = syncStatuses[0];
          syncStatus.fromBlockNumber = syncStatus.fromBlockNumber || this.initBlock!;
          syncStatus.toBlockNumber = syncStatus.toBlockNumber || this.initBlock!;
          if (
            this.initBlock! < syncStatus.fromBlockNumber ||
            this.initBlock! > syncStatus.toBlockNumber
          ) {
            throw new Error(
              `initBlock not in range of previously synced data on network ${
                this.network
              }, you may need to delete the corresponding sync status record to sync from scratch`
            );
          }
  ...
  async getVolumeUSDForBlock(
    ...
    if (!syncStatus || !syncStatus.fromBlockNumber || !syncStatus.toBlockNumber) {
    ...
    if (fromBlock < syncStatus.fromBlockNumber) {
      logger.error('getVolumeUSDForBlock: requested fromTime is before synced time');
      return {};
    }

    if (toBlock > syncStatus.toBlockNumber) {
      logger.error('getVolumeUSDForBlock: requested toTime is after synced time');
      return {};
    }
    ...
    volumes.map(
      (volume: Volume) => {
        const identifier = this.marketMakerAddressMap[volume.makerAddress]?.toLowerCase();
        if (!identifier) {
          logger.error(`getVolumeUSDForBlock: Unable to recognize address ${volume.makerAddress}`);
          return;
        }

        aggregatedVolume[identifier] = new BigNumber((volume as any).getDataValue('totalVolumeUSD') as string)
  ...
  async getVolumeAggregationUSD(period: string) {
    ...
    if (periodMS > MAX_PERIOD) throw new Error(`Invalid period: max period is ${moment.duration(MAX_PERIOD).asDays()} days`);

    const end = moment();
    const start = moment().subtract(periodMS, 'ms').utc().startOf('day');
    const periods = generatePeriods(start, end);

    return Promise.all(periods.map(async (period, i) => {
